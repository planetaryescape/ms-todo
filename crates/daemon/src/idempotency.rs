...
//!
//! Only results that may have changed something are kept: a success, an
//! `outcome_unknown`, or a failure after some tasks were changed. Any other
//! failure changed nothing (every such request is either idempotent or was
//! proved not to have run), so the key is released and a retry runs.
...
pub(crate) async fn run_once(
    ...
    let kept = match &result {
        Ok(_) => true,
        Err(error) => error.kind == ErrorKind::OutcomeUnknown.as_str() || !error.applied.is_empty(),
    };
    let stored = if kept {
