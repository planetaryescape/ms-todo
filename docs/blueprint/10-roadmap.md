...
- **8a: steps and links.** Promise: "I can break a task into steps and attach links." Demo: add three steps in the CLI, tick one on the phone, see it ticked in the TUI. `steps` and `links` in the CLI and the detail pane. A checklist-item PATCH always includes `isChecked` ([02](02-data-model.md#outbox-semantics)). **Done when:** steps and links added in ms-todo show on the phone, and a step checked on the phone shows as checked in ms-todo.
  **As built (2026-09-25, D-055):** steps and the link stay inline in the task's JSON (no new tables); each step or link written is outbox op `child`, with `If-Match` on the task's etag, `isChecked` on every step PATCH, placeholder IDs until a create is answered, and a lost create `unknown` and flagged at once. CLI `steps list|add|edit|check|uncheck|delete` and `links list|add|edit|delete`, protocol 13; the TUI's detail pane shows the steps with checkboxes and the link, with `Space`, `a`, `e`/Enter and `d` on them, and `2/5` on a task's row. S15 first, on the spike list: a link needs `applicationName`, its PATCH and DELETE honour the task's etag, a link field can't be cleared, and steps can't be reordered, so there's no `steps order`. Tests: fake-Graph CLI tests (several added in order, check, uncheck and rename with `isChecked` and `If-Match`, delete, naming by number, ID and text, a 412 resent and a conflict refused, undo with a refusal, a delete undone checked, a create with no answer left `unknown` and never resent, the one-link rule, a step checked on the phone arriving by delta), the store's placeholder swap and restart rule, the plan and undo rules (unit), and the TUI's keys and a snapshot. Latency on the demo data (debug build): cold start 7.1 ms, keypress p95 2.4 ms, view switch p95 8.9 ms. **Driven live** on the `livetest` instance, one spike task only: three steps added, one checked, unchecked, checked and renamed (still checked), one deleted (exit 2 without `--yes`), `undo` of the delete (the step back), a link added, a second refused (exit 2), renamed, `undo` (name back), set to an `ftp:` URL (kept, with the note); `raw GET` after each matched. A step checked straight on Graph (standing in for the phone) arrived with `sync --wait`. The steps and link were then deleted, the task is back to none, and the spike list to 31 tasks. **Not verified by us:** the phone showing them (BK's check).
- **8b: attachments.** Promise: "I can attach files to a task and get them back." Demo: attach a PDF in the CLI, open it on the phone, download it back. Direct upload, upload sessions, download (safe filenames, `.part` then rename, 0600), and the final-chunk `unknown` rule ([03](03-graph-provider.md#endpoints-the-whole-surface)). **Done when:** a 10 MB attachment uploads and downloads with identical bytes (compare the sha256), and it opens on the phone.
  **As built (2026-09-25, D-056):** attachments' metadata inline in the task's JSON under `attachments` (no table), fetched by sync in `$batch` (migration `0007` reads whole the lists that already had some); an add or delete is a `child` outbox operation, the add reading its file when sent and refusing one changed since; an upload session resumes after a lost chunk answer (S16: `InvalidStart` means it landed), and a lost last answer is `unknown` and flagged; a delete keeps a 0600 copy for a week so `undo` re-attaches it; downloads written safely by the daemon. CLI `attachments list|add|download|delete`, protocol 14; the TUI's Files in the detail pane, `A`, Enter/`o` to save and open, `d`, `[attachments] download_dir`, and `⎘` on a row. S16 first, on one spike task. Tests: fake-Graph CLI tests (a small POST; a 10 MB session in order with the token; a lost chunk answer resent and the upload finished in one attempt; a lost last answer `unknown`, flagged and never resent; over 25 MB refused before any request; a relative path and a dry run; downloads with a `..` name made safe, numbered, 0600, `--force`, no `.part` left, a symlinked directory refused; delete and undo from the kept copy with the same sha256, and undo of the undo; undo of an add; a phone's file arriving and leaving by sync), the store's carry rule and migration, the safe-write and kept-copy rules and a file changed after queueing (unit), Graph's refusal over 25 MB and a lost POST answer, and the TUI's keys, download-and-open and a snapshot. Latency on the demo data (debug build): cold start 7.2 ms, keypress p95 1.8 ms, view switch p95 8.0 ms. **Driven live** on the `livetest` instance, one spike task only: the first sync fetched the lists of the 24 cached tasks that had attachments; a 48-byte text file (one POST) and a 10 MB random file (an upload session) attached, listed, downloaded (0600) with sha256 equal to the originals for both; `delete` without `--yes` exited 2, with it the file left Graph, `undo` re-attached it from the kept copy with the same sha256; the run repeated after a code-quality pass with the same results; both files then deleted, the task back to none (`raw GET`: no attachments, `hasAttachments` false). The spike list gained a 32nd task during the run that ms-todo didn't make: a new "S12 daily recurring" at 08:48 UTC, as the To Do apps make one when that daily task is completed; left alone. **Not verified by us:** the file opening on the phone (BK's check).
...
- **8d: assignment.** Promise: "I can track who I'm waiting on." Demo: assign a task, then see it in the TUI's Assigned view. Assignment (`--assignee`) and the Assigned view; folders moved to rung 5c (D-047). **Done when:** `tasks list --assignee` returns the right tasks.
  **As built (2026-09-25, D-057):** `assignee` and `assigneeStatusSet` in the task's extension through the existing `task_extension` merge write; setting one makes an open task `waitingOnOthers` (a status edit with an `expect_fields: { status }` precondition, then the flag only if the edit was made), and clearing it makes the task `notStarted` again only while ms-todo's `waitingOnOthers` still holds. `--assignee`, `--clear-assignee` and `--keep-status` on `tasks add` and `tasks edit`, `tasks list --assignee NAME|'*'`, `waiting [PERSON]`; the TUI's Assigned view grouped by person, a person chip on rows, the Assignee field in the detail pane, `W` "Assign to…". Protocol 15. No quick-add token (`@` is a category). Tests: the plans (unit), the store's view and count, the CLI end to end against the fake Graph (set and clear with the pairing, a status the user chose or changed kept, one changed mid-send skipped and never flagged, the merge keeping `myDay`, `opId` and an unknown field, create, filters and formats, dry run, several tasks, undo with the changed-since rule and the assignee's pairing, another machine's assignment by sync), the TUI's prompt, field, view and snapshots. Latency on the demo data (debug build): cold start 8.0 ms, keypress p95 2.4 ms, view switch p95 9.7 ms. **Driven live** on the `livetest` instance, spike list only (My Day's day pinned to 2000-01-01 so no rollover touched anything): one spike task assigned to Sam came back from Graph `waitingOnOthers` with `assignee`, `assigneeStatusSet` and its old `opId`; `tasks list --assignee sam` and `waiting` found it (and the one S2 spike task already assigned); clearing left `opId` alone and `notStarted`; `undo` of the clear put all three back; a last clear left the list as before (32 tasks, 28 open, 4 completed, every task's extension fields unchanged) and the instance was deleted.
- **8e: categories, extensions, lists and the remaining task fields.** Promise: "Anything the To Do API can do, ms-todo can do." Demo: create a list, recolour a category, and set a start date and a recurrence from the CLI. `categories list|create|recolor|delete`, `extensions`, `lists create|rename|delete`, start dates and recurrence editing. **Done when:** every command in [07](07-cli.md) is implemented, has wiremock tests, and has run live against a throwaway list, behind a feature-gated live smoke test.
  **As built (2026-09-25, D-058):** every command in 07 (`sync --list` dropped). Lists made, renamed and deleted through the outbox (ops `list_create`, `list_update`, `list_delete`; a task can go into a list not yet created; undo within the changed-since rule, and an empty list's delete undone by making it again); categories and open extensions straight to Graph with dry runs, idempotency keys and undo (recorded as `remote` outbox rows); `--start`, `--recur`, `--category`, `--body-file` and `--strict` on `tasks add` and `tasks edit`; `tasks list` filters (`--status`, `--due`, `--importance`, `--category`, `--sort`, `--limit`, every list when there's no `--list`); `tasks show`, `lists show`; global `--fresh`, `--quiet`, `--no-color`; `daemon restart` and `daemon logs [--follow]`. Protocol 16. The TUI's palette makes, renames and deletes lists, and rows show `@category` chips. Tests: fake-Graph CLI tests for lists, categories, extensions, the task fields and the filters, the store's and daemon's rules (unit), the TUI's list prompts and chips. Found live first, on the `livetest` instance: Graph lists no extensions (a list's collection is a 404 too), a start date sent alone keeps a due date the task has, and a recurrence counts from the start date, moving it and the due date (so a start on a repeating task is refused, and a recurrence moves the start to its first occurrence). **Driven live** on `livetest`, with throwaway lists and a category named `ms-todo-spike-8e-*` only (the spike list untouched, 32 tasks): a list created, renamed, undone, deleted without `--yes` (exit 2), deleted and undone (made again with a new Graph ID, the same local ID); `lists rename Tasks` refused; a category created, a duplicate refused, recoloured and undone, deleted and undone (back with its colour); a start date set, kept with a due date, cleared; a recurrence set on a task with a start date (start and due both on the first Monday), cleared, undone; `--due overdue`, `--due "before tomorrow" --status open`, `--status completed|open|waiting` across the account (read only). The live smoke test (`cargo test --features live-smoke --test live_smoke -- --ignored`) then ran the whole surface once and passed; its first runs found that Graph can't list a list's extensions (fixed) and issue 005 (a reopened task's reminder stays off, and its move then fails safely; not fixed here). Every throwaway list and category was deleted: 30 lists and 7 categories, as before, and the instance removed.
