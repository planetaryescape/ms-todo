...
- `--instance <name>`, `--fresh`, `--quiet`, `--no-color` (built in rung 8e, D-058: `--fresh` syncs and waits before a read; `--quiet` prints nothing on stderr but errors; `--no-color` turns off bold, as `NO_COLOR` does).
...
**The rest of the API, as built (rung 8e, D-058).** Every command above is built.

- `lists show L` (the list with `open_count` and `completed_count`), `lists create NAME [--folder F]`, `lists rename L NAME`, `lists delete L [--yes]`: through the outbox as folder writes are, with `--dry-run` (`lists: [{ id, name, changes }]`; a delete's `changes` are `{ deleted, tasks, undoable }`) and `--idempotency-key`, answering `Applied` with actions `create_list`, `rename_list`, `delete_list`. "Tasks" and Flagged Emails are refused (exit 2), and so is a name another list has. `undo` reverses a create (while the list is empty and keeps its name), a rename (while the name holds) and an empty list's delete; a delete of a list that had tasks can't be undone (exit 7).
- `tasks show T [--list L]`: one task, every field, in every format.
- `tasks list` filters: `--status open|completed|all|not-started|in-progress|waiting|deferred` (and `--completed`), `--due today|overdue|none|any|"before W"|"after W"|W`, `--importance`, `--category`, `--sort due|importance|created|modified|title`, `--limit N`. A filter with no `--list` looks in every list, soonest due first, each task with `list`; its table is `DONE, DUE, IMPORTANT, LIST, TITLE` and its CSV the task columns then `list`.
- `tasks add` and `tasks edit` take `--start` (edit: `--clear-start`), `--recur "every …"` (edit: `--clear-recur`), `--category C`... (edit: replacing; `--clear-categories`) and `--body-file F`; `tasks add --strict`. A start date goes with the task's due date, or becomes it; a recurrence's first occurrence is the due date sent, with `recurrenceTimeZone`; a start date on a repeating task is refused, and a recurrence moves a start date to its first occurrence (Graph counts a recurrence from the start date). **Protocol 16:** `TaskEdit.start`, `recurrence`, `categories`.
- `categories list | create NAME [--color C] | recolor CAT --color C | delete CAT [--yes]`: C is `preset0`–`preset24` or `none`; CAT a name (ignoring case) or ID. `extensions list|get|set|delete (list|task) ID [--list L] …`, `set` with `--json '{…}'` (an object, not ms-todo's own extension). Both go straight to Graph (not cached), with `--dry-run` (`changes: { target, body }`), `--idempotency-key` and `undo`; answers are `Applied` with actions `category_create`, `category_recolor`, `category_delete`, `extension_set`, `extension_delete` and the category or extension as items. Collections of them carry no `sync`.
- `daemon restart`; `daemon logs [--follow]` (JSON `{ path, lines }`; following, one `{ line }` per line).
