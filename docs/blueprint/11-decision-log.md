...
- **Idempotency keys live in their own table**, `idempotency_keys`, since there's no outbox to hang them on. A key keeps its result only when the request may have changed something: a success, an `outcome_unknown`, or a failure after some tasks were changed. Any other failure changed nothing (every such request is idempotent or was proved not to have run), so the key is released and the agent can retry with it. After a restart, a key whose request never finished gets an `outcome_unknown` result, kept for 24 hours.
