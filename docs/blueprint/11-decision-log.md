...
- **`done`** is a new read, `CompletedTasks { since, until, list, folder, limit }`, answered from the cache through the daemon, never from Graph. `completedDateTime` is a **date**: Graph keeps midnight UTC of the day (S12; live on 2026-09-25, a completion at 03:29 BST was `2026-09-25T00:00:00Z`), so it's read by the due dates' nearest-midnight rounding (the daemon's `graph_completion_date`, and the TUI's reading of the same field), and no time is shown or claimed. Not verified: which day Graph records for a completion made between midnight and 01:00 BST, when the UTC date is the day before; if it's the UTC day, `done` shows the day before. A completion Graph hasn't answered has no date yet: it counts as today's, comes first ("Not synced yet", `completed_on` null), and the Completed view's order now puts it first too (it was last), matching the TUI's list order.
