...
3. Phase 0: the Entra app is already registered. Read its client ID from `../setup/entra-app-registration.md` or the `ms-todo Entra app` item in 1Password's `Environment Variables` vault; the local `.env` on the registration machine is Git-ignored and is not present in a fresh clone. Device-code sign-in and Graph access were verified on 2026-09-24 (S5 in `12-open-questions.md`). Run the remaining spikes against a throwaway list in my account. Record the evidence and conclusions in that file, and update any blueprint documents whose assumptions changed. Stop and report before phase 1, listing any results that change the design.
