...
- **Store times as Graph sends them.** Graph returns `dateTimeTimeZone` (a local time plus a zone name) for due, start, reminder and completed dates. Store both the raw pair and a UTC value computed from it for sorting and filtering.
...
tasks(local_id PK, graph_id UNIQUE NULL, list_local_id FK, title,
      ...
      is_reminder_on, reminder_at_utc NULL, due_at_utc NULL, start_at_utc NULL,
...
outbox(op_id PK, created_at, entity_kind, entity_local_id, op TEXT, payload_json,
      depends_on_op_id NULL, attempts, next_attempt_at, state
      'pending'|'inflight'|'failed'|'done', last_error NULL,
...
Index what the TUI views need to be instant: `tasks(list_local_id, status, deleted_at)`, `tasks(due_at_utc)`, `tasks(my_day_date)`, `tasks(importance)`, plus an FTS5 table over `tasks(title, body_content)` for search. FTS5 is enough at To Do's scale; we're not using Tantivy (D-011).
