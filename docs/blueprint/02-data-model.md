...
Rung 3a built `lists`, `tasks`, `sync_state`, an `idempotency_keys` table and a `counters` table; the schema as built is `crates/store/migrations/0001_lists_tasks_sync_state.sql`, and [D-036](11-decision-log.md) explains where it differs from this draft. Rung 4 added `outbox` (`0003_outbox.sql`, [D-040](11-decision-log.md)). The other tables arrive with the rungs that use them. Rung 8a kept steps and links inline in the task's `raw_json`, as Graph sends them, instead of the `checklist_items` and `linked_resources` tables below ([D-055](11-decision-log.md)). Rung 8b keeps attachments' metadata there too, under `attachments`, instead of the `attachments` table ([D-056](11-decision-log.md)).
...
- **A checklist-item PATCH always includes `isChecked`.** Leaving it out resets the step to unchecked (S1). A write to a step or the link is outbox op `child`; one that creates a child caches it under a `local-…` ID until Graph's answer gives the real one (D-055). An attachment's add and delete are `child` operations too; the add reads its file when sent (D-056).
