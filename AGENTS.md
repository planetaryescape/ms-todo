...
**Status: Rung 5a: TUI** is built, on top of rung 4b (search), rung 4 (offline and undo), rung 3b (live sync), rung 3a (instant reads), rung 2 (capture and finish tasks), rung 1 (see my tasks) and F1 (install and sign in). The workspace is `crates/core`, `crates/protocol`, `crates/graph` (sign-in, the HTTP client, endpoints, `$batch`), `crates/store` (SQLite through sqlx; only the daemon uses it), `crates/daemon`, `crates/cli` and `crates/tui` (ratatui; depends on the protocol and core only, and starts from the daemon's `Seed`, then follows the events of `Subscribe`, D-043). The daemon keeps a cache in step with Graph by delta sync, with a whole read of a scope as the first pass and the reset path (on start, every 20 seconds while a client is active, every 5 minutes otherwise, on `sync`, and after the outbox sends), and answers reads from it. Writes are applied to the cache and queued in the outbox in one transaction and answer at once; the daemon's outbox worker sends them (`crates/daemon/src/outbox/`), with `unknown` outcomes attributed by `opId` and never resent by itself (D-028, D-040). Entities carry local IDs with `graph_id` and `sync_state` beside them, at `schema_version` 2. Search is SQLite FTS5 over each task's title and plain-text notes, kept current by triggers (migration `0004`, D-041). The CLI has `auth`, `lists list`, `tasks list [--search Q]|add|complete|reopen|edit|delete`, `search`, `outbox list|retry|discard`, `undo`, `sync [--wait]`, `doctor`, `schema [CMD]`, `raw GET|POST|PATCH|DELETE`, `daemon start|stop|status`, `--dry-run`, `--idempotency-key`, and `--format table|json|jsonl|ids|csv`. `tui` opens the TUI (`--ascii`, `--bench-startup`). The agent skill is `skills/ms-todo/SKILL.md` (v3). Rung 5b (field editing, multi-select, the palette, diagnostics and Homebrew) is next; what rungs 3a, 3b and 4 didn't build is listed in D-036, D-038 and D-040, and semantic search is deferred (D-042). The blueprint was written in the planning session on 2026-09-24, on a different machine from the build.
