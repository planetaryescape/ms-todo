...
**Status: Rung 3b: live sync** is built, on top of rung 3a (instant reads), rung 2 (capture and finish tasks), rung 1 (see my tasks) and F1 (install and sign in). The workspace is `crates/core`, `crates/protocol`, `crates/graph` (sign-in, the HTTP client, endpoints, `$batch`), `crates/store` (SQLite through sqlx; only the daemon uses it), `crates/daemon` and `crates/cli`. The daemon keeps a cache in step with Graph by delta sync, with a whole read of a scope as the first pass and the reset path (on start, every 20 seconds while a client is active, every 5 minutes otherwise, on `sync`), and answers reads from it; writes go synchronously to Graph and then into the cache (no outbox yet). Entities carry local IDs with `graph_id` beside them, at `schema_version` 2. The CLI has `auth`, `lists list`, `tasks list|add|complete|reopen|edit|delete`, `sync [--wait]`, `doctor`, `schema [CMD]`, `raw GET|POST|PATCH|DELETE`, `daemon start|stop|status`, `--dry-run`, `--idempotency-key`, and `--format table|json|jsonl|ids|csv`. The agent skill is `skills/ms-todo/SKILL.md` (v1). Rung 4 (the outbox: offline writes, never lose a write) is next; what rungs 3a and 3b didn't build is listed in D-036 and D-038. The blueprint was written in the planning session on 2026-09-24, on a different machine from the build.
